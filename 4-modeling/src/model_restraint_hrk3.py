...
class MyModel(automodel):
     def special_patches(self, aln):
         self.rename_segments(segment_ids=['A','B'], renumber_residues=[1, 63])
...
#       Use a harmonic potential and X-Y distance group.
         rsr.add(forms.gaussian(group=physical.xy_distance,
                                 feature=features.distance(at['CA:74:A'],		# Val74 on Bcl2a1 (PDB numbering)
                                                          at['CA:72:B']),	# Leu72 on Hrk - Leu79 on PUMA (PDB numbering)
                                mean=7.0, stdev=0.1))

a = MyModel(env,
            alnfile  = '../data/5uulpuma_bcl2a1hrk_3.ali',    # alignment filename
            knowns   = '5uul_temp.pdb',    # known template structure
            sequence = 'bcl2a1_hrk_3',     # target sequence
